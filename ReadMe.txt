...
Main structure
----------------
The main structure used is an CEV_GifAnim, this to store all the informations.
...
an CEV_GifAnim structure.

Any created structure should be freed using CEV_gifAnimFree().
...
return value :
- on success, pointer to an CEV_GifAnim structure is returned.
- a NULL pointer is returned if function has failed, stderr is completed with error information.

The main texture (as returned by CEV_gifTexture()) shows the first logical frame if displayed as it is.

Animation mode is set to GIF_STOP when loaded, you must enable animation by setting
the animation mode through CEV_gifLoopMode().
...
frees allocated CEV_GifAnim structure.
...
Basic Code :
	...
	animation = CEV_gifAnimLoad("MS0.gif", myRender);

	SDL_Texture *actTexture = CEV_gifTexture(animation);
...
If you stop calling CEV_gifAnimAuto(), no animation occurs, and is just freezed as it
until further update.
The main texture will keep displaying what it was displaying before you stopped calling CEV_gifAnimAuto().
...
----------------------------------------------------------
void CEV_gifLoopMode(CEV_GifAnim* anim, unsigned int mode)
----------------------------------------------------------
...
mode can get any of these values :
 ...
 GIF_ONCE_FOR    : gif will run from first frame to latest then stop, see CEV_gifLoopReset() to restart animation.
...
play the animation faster by calling CEV_gifFrameNext every 100 ms.
...
a call to CEV_gifReverse will set it to GIF_REPEAT_REV. 
...
until a call to CEV_gifLoopReset() is done.
...
----------------------------------
char CEV_gifMethod(CEV_GifAnim* anim, uint8_t num)
----------------------------------
...
----------------------------------
void CEV_gifMethodSet(CEV_GifAnim* anim, int num, uint8_t method)
----------------------------------
...
- CEV_gifAnimAuto() now ticks on every pic change.
